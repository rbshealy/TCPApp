#
# configure_camera_feature.py (for Python 3)
#
...
def set_exposure_time():
    ...
    # Get ExposureTime (minimum and maximum).
    status, exposure_time_min, exposure_time_max = cam_device.cam_control.get_exposure_time_min_max()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_exposure_time_min_max, status = {status}\n")
        return False
    ...
    # Set ExposureTime.
    status = cam_device.cam_control.set_exposure_time(exposure_time)
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed set_exposure_time, status = {status}\n")
        return False

    # Get ExposureTime (current).
    status, exposure_time = cam_device.cam_control.get_exposure_time()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_exposure_time, status = {status}\n")
        return False
...
def set_gain():
    ...
    # Get Gain (minimum and maximum).
    status, gain_min, gain_max = cam_device.cam_control.get_gain_min_max()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_gain_min_max, status = {status}\n")
        return False
    ...
    # Set Gain.
    status = cam_device.cam_control.set_gain(gain)
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed set_gain, status = {status}\n")
        return False

    # Get Gain (current).
    status, gain = cam_device.cam_control.get_gain()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_gain, status = {status}\n")
        return False
...
def set_frame_rate():
    ...
    # Get AcquisitionFrameRate (minimum and maximum).
    status, frame_rate_min, frame_rate_max = cam_device.cam_control.get_acquisition_frame_rate_min_max()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_acquisition_frame_rate_min_max, status = {status}\n")
        return False
    ...
    status = cam_device.cam_control.set_acquisition_frame_rate(frame_rate)
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed set_acquisition_frame_rate, status = {status}\n")
        return False

    # Get AcquisitionFrameRate (current).
    status, frame_rate = cam_device.cam_control.get_acquisition_frame_rate()
    if status != pytelicam.CamApiStatus.Success:
        print(f"Failed get_acquisition_frame_rate, status = {status}\n")
        return False
